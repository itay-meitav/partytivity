...
export const sendReset = async (req: Request, res: Response) => {
  ...
  if (checkUser) {
    ...
    return res
      .cookie(
        "reset",
        jwt.sign({ id: checkUser.id }, envConfig.jwt.JWT_SECRET, {
          expiresIn: "10m",
        }),
        {
...
export const checkResetToken = async (req: Request, res: Response) => {
  ...
  if (token && cookie) {
    try {
      jwt.verify(token, envConfig.jwt.JWT_SECRET);
      return res.json({
        success: true,
      });
    } catch (error) {
...
async function sendVerificationEmail(
  ...
  const token = jwt.sign({ email: checkUser.email }, envConfig.jwt.JWT_SECRET, {
    expiresIn: "24h",
  });
  ...
  return res
    .cookie(
      "verify",
      jwt.sign({ name: checkUser.name }, envConfig.jwt.JWT_SECRET, {
        expiresIn: "10m",
      }),
      {
        expires: new Date(Date.now() + 1000 * 60 * 10),
